...
# ╔═╡ 637b5360-f544-11eb-3bd8-a39766a6ca91
begin
...
# ╔═╡ a18aa448-5767-42c3-8e99-bfa188baab1a
md""" ## Redes Neurais Densas para classificação de MNIST 1"""

# ╔═╡ c16d0a97-e505-4a13-aeaf-1d9173b0ef8d
begin
	imgs = MNIST.images() #carrega as imagens 
	colorview(Gray, imgs[5]) #vizualiza a 3a imagem do banco de dados em tons de cinza
end

# ╔═╡ 9cfbddd4-dbcb-4265-accc-9b6e632058cd
typeof(imgs)

# ╔═╡ 148cc509-d452-4c36-b3ab-3d762fee7c66
size(imgs)

# ╔═╡ d7668f63-e6fc-43c4-9930-e30e50747e6d
begin
	imgs[3][:,15]
end

# ╔═╡ 729ccb3d-72f8-4910-a219-698cfdefee43
begin
	myFloat32(x) = Float32.(x)
	fpt_imgs = myFloat32.(imgs)
end

# ╔═╡ dd22c15a-f878-401e-bc08-7b80a976fe78
begin
	vectorize(x) =  x[:]
	vectorized_images = vectorize.(fpt_imgs)
end

# ╔═╡ 76393534-770a-4211-88f1-1115b1bbce70
begin
	X = hcat(vectorized_images...)
	typeof(X)
end

# ╔═╡ 403c48b3-d3b1-4993-8837-c616f7fac5bd
begin
	onefigure = X[:,5] #carrega uma coluna inteira em onefigure
	t1 = reshape(onefigure,28,28)
	colorview(Gray,t1)
end

# ╔═╡ 4732b2b6-c367-4d88-8f4d-7664afeb34fa
labels = MNIST.labels()

# ╔═╡ 2d4bfc67-3230-49d7-837d-f31cc2a69cc2
Y = onehotbatch(labels, 0:9)

# ╔═╡ ae990765-13ac-4af9-9a86-d7ba6898a98c
begin
	m = Chain( #cria uma rede neural em cascata
		Dense(28*28,32,relu), #primeira camada de 28x28 neuroniso de entrada e 32 de saida
		Dense(32,10,relu), #segunda camada de 32 neurônios de entrada e 10 de saida
		softmax) #camada que ajusta as saídas para ter soma total = 1
end

# ╔═╡ bc0e9bfc-4304-4ddb-9050-9fefc03ca518
m(onefigure) #aplica uma figura no modelo e tem estas saídas

# ╔═╡ b36b1a78-911d-4fd0-998b-6c8248e84c16
begin
	loss(x,y) = Flux.crossentropy(m(x) , y) #cria função de perda/custo
	accuracy(x,y) = mean(argmax(m(x)) .== argmax(y)) #declara função de acuracia
end

# ╔═╡ aa5f98d2-d9ce-40ca-bd5d-76ee6405a9d1
begin
	datasetx = repeated((X,Y),200)
	C = collect(datasetx)
end

# ╔═╡ c262936e-bb5a-40e7-8274-4facee73de81
evalcb = () -> @show(loss(X,Y))

# ╔═╡ bc1fcc94-fbd0-4c32-80a2-861b9022d1c4
ps = Flux.params(m)

# ╔═╡ 87e0ff19-bb6e-40e9-a6fd-360db66a0797
begin
	global grafico=[]
	opt = ADAM()
	Flux.train!(loss, ps, datasetx, opt, cb = throttle(evalcb,20))
end

# ╔═╡ d8e5b707-5019-4206-83c9-0fb6a76cc541
evalcb2 = () -> push!(grafico,loss(X,Y))

# ╔═╡ bed7546e-8a74-4179-8fca-94d653923f7b
begin
	tX = hcat(float32.(reshape.(MNIST.images(:test),:)) ...)
	test_image = m(tX[:,5])
	test = argmax(test_image) -1
end

# ╔═╡ 8ab505ec-9ffa-4a6d-b06a-13adffcb3055
begin
	image_test = reshape(tX[:,5],28,28)
	colorview(Gray, image_test)
end

# ╔═╡ b35e12e5-218f-498f-b9ee-8e236f9df40f
cb
