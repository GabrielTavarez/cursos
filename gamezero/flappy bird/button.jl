...
#construtor
function Button(x, y, width, height, text = "" ; font = "freepixel", font_size = 20, font_color = [0,0,0,255], color = colorant"white", border = false, border_stroke = 0, border_color=colorant"black", semipresscolor = RGB(0,0,0), latch = false)

    text_actor =  TextActor(text, font, font_size = font_size, color = font_color)
    text_actor.top = y + (height - text_actor.position.h)/2
    text_actor.left = x + (width - text_actor.position.w)/2
    releasecolor = color
